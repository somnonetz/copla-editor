...
let TickList; // the ' = undefined' keeps jshint happy.
 ...
 *  ): TickList}
 */
let Ticker; // the ' = undefined' keeps jshint happy.

/** @type {Ticker} */
export var numericLinearTicks = function (a, b, pixels, opts, dygraph, vals) {
   const nonLogscaleOpts = function (opt) {
...
export var numericTicks = function (a, b, pixels, opts, dygraph, vals) {
   const pixels_per_tick = /** @type{number} */(opts('pixelsPerLabel'));
   const ticks = [];
   let i;
   let j;
   let tickV;
      ...
      if (opts('logscale')) {
         ...
         if (maxIdx - minIdx >= nTicks / 4) {
            for (let idx = maxIdx; idx >= minIdx; idx--) {
               ...
               const pixel_coord = Math.log(tickValue / a) / Math.log(b / a) * pixels;
               const tick = { v: tickValue };
               ...
               else if (Math.abs(pixel_coord - lastDisplayed.pixel_coord) >= pixels_per_tick) {
                  lastDisplayed = {
                     tickValue,
                     pixel_coord,
                  };
...
var pickDateTickGranularity = function (a, b, pixels, opts) {
   ...
   for (let i = 0; i < Granularity.NUM_GRANULARITIES; i++) {
      const num_ticks = numDateTicks(a, b, i);
...
export var getDateAxis = function (start_time, end_time, granularity, opts, dg) {
   ...
   const ticks = [];
   let tick_date = accessors.makeDate.apply(null, date_array);
