...
class DygraphLayout {
   ...
   setAnnotations(ann) {
      ...
      this.annotations = [];
      const parse = this.dygraph_.getOption('xValueParser') || function (x) { return x; };
   ...
   _evaluateLimits() {
      const xlimits = this.dygraph_.xAxisRange();
   ...
   static calcYNormal_(axis, value, logscale) {
      if (logscale) {
         const x = 1.0 - ((utils.log10(value) - utils.log10(axis.minyval)) * axis.ylogscale);
   ...
   _evaluateLineCharts() {
      const isStacked = this.dygraph_.getOption('stackedGraph');
      const isLogscaleForX = this.dygraph_.getOptionForAxis('logscale', 'x');

      for (let setIdx = 0; setIdx < this.points.length; setIdx++) {
         const points = this.points[setIdx];
         const setName = this.setNames[setIdx];
         const connectSeparated = this.dygraph_.getOption('connectSeparatedPoints', setName);
         const axis = this.dygraph_.axisPropertiesForSeries(setName);
         // TODO (konigsberg): use optionsForAxis instead.
         const logscale = this.dygraph_.attributes_.getForSeries('logscale', setName);
