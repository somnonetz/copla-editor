...
class EDF {
  ...
  async read(fromMs, tillMs) {
    ...
    window.measure.log('readMs', tillMs - fromMs);
    window.measure.log('readBlocks', tillBlock - fromBlock);
    window.measure.log('readBytes', result.byteLength);
  ...
  async getData(options = {}) {
    ...
    const till = Math.min(options.till || Number.MAX_SAFE_INTEGER, header.numberOfDataRecords * header.recordDurationTime * 1000); // bis ende

    if (till <= from) throw new Error('Bad interval');

    window.measure.start('loadMs');
    const data = await this.read(from, till);
    window.measure.stop('loadMs');
    window.measure.start('parseMs');
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const decimation = options.frequency
        ...
        : 1;
      const date = index => 0 + from + index * xScale; // timestamps are relative to `header.start`
      ...
      if (decimation === 1) {
        ...
        for (let j = 0; j < data[i].length; j++) {
          buffer[i].push(date(j));
          buffer[i].push(getValue(data[i][j]));
          // buffer[i].push([
          //   /* X */ date(j),
          //   /* Y */ getValue(data[i][j]),
        ...
        buffer[i] = []; // date, min, max, date, min, max ...
        let min;
        let max;
        ...
        for (let j = 0; j < data[i].length; j++) {
          ...
          if (j === 0) {
            ...
            // buffer[i].push([date(j), value, value]);
            buffer[i].push(date(j));
            buffer[i].push(value);
            buffer[i].push(value);
          }
