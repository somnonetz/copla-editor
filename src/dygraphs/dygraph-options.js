...
class DygraphOptions {
   ...
   reparseSeries() {
      ...
      for (let idx = 0; idx < this.labels_.length; idx++) {
         ...
         const yAxis = DygraphOptions.axisToIndex_(optionsForSeries.axis);
...
*/
   validateOptions_() {
      ...
      const that = this;
      const validateOption = function (optionName) {
         if (!OPTIONS_REFERENCE[optionName]) {
            that.warnInvalidOption_(optionName);
         }
      };

      const optionsDicts = [this.xAxis_.options,
...
if (typeof (process) !== 'undefined') {
   if (process.env.NODE_ENV != 'production') {
      const WARNINGS = {}; // Only show any particular warning once.
   }
}
