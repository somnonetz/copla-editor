...
class Legend {
   ...
   activate(g) {
      ...
      if (userLabelsDiv && userLabelsDiv !== null) {
   ...
   select(e) {
      ...
      if (legendMode === 'follow') {
         ...
         let leftLegend = points[0].x * area.w + 50;
         const topLegend = points[0].y * area.h - 50;
   ...
   deselect(e) {
      ...
      // nah, we don't
      if (!this.hasOwnProperty('one_em_width_')) {
         this.one_em_width_ = calculateEmWidthInDiv(this.legend_div_);
      }

      const html = Legend.generateLegendHTML(e.dygraph, undefined, undefined, this.one_em_width_, null);
      this.legend_div_.innerHTML = html;
   }
    ...
    * @private
    */
   static generateLegendHTML(g, x, sel_points, oneEmWidth, row) {
      ...
      if (typeof (x) !== 'undefined') {
         ...
         for (i = 0; i < sel_points.length; i++) {
            ...
            const yHTML = fmtFunc.call(g, pt.yval, yOptView, pt.name, g, row, labels.indexOf(pt.name));
      ...
      const formatter = (g.getOption('legendFormatter') || Legend.defaultFormatter);
      return formatter.call(g, data);
   }

   static defaultFormatter(data) {
...
const calculateEmWidthInDiv = function (div) {
