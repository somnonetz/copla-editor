...
export function isPixelChangingOptionList(labels, attrs) {
   ...
   const scanFlatOptions = function (options) {
   ...
   for (const property in attrs) {
      ...
      // This is twice-nested options list.
         const perSeries = attrs[property];
...
export var Circles = {
   DEFAULT(g, name, ctx, canvasx, canvasy, color, radius) {
      ctx.beginPath();
      ctx.fillStyle = color;
...
export function isNodeContainedBy(containee, container) {
   ...
   let containeeNode = /** @type {Node} */ (containee);
...
export function toRGB_(colorStr) {
   ...
   colorCache[colorStr] = parseRGBA(rgbStr);
...
export function numberValueFormatter(x, opts) {
   ...
   if (kmb || kmg2) {
      ...
      for (let j = k_labels.length - 1; j >= 0; j--, n /= k) {
         if (absx >= n) {
