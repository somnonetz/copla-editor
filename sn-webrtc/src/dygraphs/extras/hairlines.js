 ...
 */
  ...
  class hairlines {
    ...
    detachLabels() {
      for (let i = 0; i < this.hairlines_.length; i++) {
        const h = this.hairlines_[i];
        $(h.lineDiv).remove();
        $(h.infoDiv).remove();
    ...
    createHairline(props) {
      let h;
      const self = this;

      const $lineContainerDiv = $('<div/>').css({
        width: '6px',
        'margin-left': '-3px',
        position: 'absolute',
        'z-index': '10',
      ...
      const $infoDiv = $('#hairline-template').clone().removeAttr('id').css({
        position: 'absolute',
      })
        .show();
      ...
      const that = this;
      $infoDiv.on('click', '.hairline-kill-button', (e) => {
    ...
    updateHairlineDivPositions() {
      ...
      $.each(this.hairlines_, (idx, h) => {
        const left = g.toDomXCoord(h.xval);
    ...
    static findPrevNextRows(g, xval, col) {
      let prevRow = null;
      let nextRow = null;
      ...
      for (let row = 0; row < numRows; row++) {
        ...
        const rowXval = g.getValue(row, 0);
      ...
      $.each(this.hairlines_, (idx, h) => {
        ...
        if (!h.interpolated) {
          ...
          for (var i = 1; i < g.numColumns(); i++) {
            ...
            const prevX = g.getValue(prevRow, 0);

            const nextX = g.getValue(nextRow, 0);
            const prevY = g.getValue(prevRow, i);
            const nextY = g.getValue(nextRow, i);
            const frac = prevRow == nextRow ? 0 : (h.xval - prevX) / (nextX - prevX);
        ...
        if (that.divFiller_) {
          ...
          const html = Dygraph.Plugins.Legend.generateLegendHTML(g, h.xval, selPoints, 10);
    ...
    click(e) {
      ...
      const xval = this.dygraph_.toDataXCoord(e.canvasx);
