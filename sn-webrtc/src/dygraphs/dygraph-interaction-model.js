...
DygraphInteraction.treatMouseOpAsClick = function (g, event, context) {
   ...
   for (let i = 0; i < g.selPoints_.length; i++) {
      ...
      const distance = Math.pow(p.canvasx - context.dragEndX, 2) +
...
DygraphInteraction.endZoom = function (event, g, context) {
   ...
   if (context.regionWidth >= 10 &&
      ...
      let bottom = Math.max(context.dragStartY, context.dragEndY);
...
DygraphInteraction.startTouch = function (event, g, context) {
   ...
   if (touches.length == 1) {
      ...
      let initialAngle = 180 / Math.PI * Math.atan2(
...
const distanceFromInterval = function (x, left, right) {
   if (x < left) {
      return left - x;
   }
   else if (x > right) {
 ...
 */
const distanceFromChart = function (event, g) {
   ...
   mousedown(event, g, context) {
      ...
      const mousemove = function (event) {
         if (context.isZooming) {
            ...
            const d = distanceFromChart(event, g);
            if (d < DRAG_EDGE_MARGIN) {
               DygraphInteraction.moveZoom(event, g, context);
            }
            else if (context.dragEndX !== null) {
